...
const PopoverContent = React.forwardRef(({ className, align = "center", sideOffset = 4, ...props }, ref) => (
    <PopoverPrimitive.Portal>
        <PopoverPrimitive.Content
            ...
            sideOffset={sideOffset}
            className={cn(
                "z-50 w-64 rounded-xl bg-obsidian-900 p-2 text-white shadow-md outline-none animate-in data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:zoom-in-95 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
                className
            )}
            {...props}
        />
...
const DialogContent = React.forwardRef(({ className, children, ...props }, ref) => (
    <DialogPortal>
        ...
        <DialogPrimitive.Content
            ...
            {...props}
        >
            <div className="relative bg-obsidian-900 rounded-[28px] overflow-hidden shadow-2xl p-1">
                {children}
                <DialogPrimitive.Close className="absolute right-3 top-3 z-10 rounded-full bg-obsidian-950 p-1 hover:bg-obsidian-800 transition-all">
                    <XIcon className="h-5 w-5 text-obsidian-300 hover:text-white" />
...
export const PromptBox = React.forwardRef(({ className, value, onChange, onSubmit, ...props }, ref) => {
    ...
    // Add keyDown support for submitting with Enter (but Shift+Enter for newline)
    const handleKeyDown = (e) => {
        if (e.key === "Enter" && !e.shiftKey) {
            e.preventDefault();
            if (onSubmit && hasValue) {
                onSubmit(e);
                handleValueChange(""); // reset value if controlled
            }
        }
    };
    ...
    const hasValue = actualValue.trim().length > 0 || imagePreview;
    ...
    return (
        <div
            className={cn(
                "flex flex-col rounded-3xl p-2 shadow-sm transition-colors bg-obsidian-950 border border-obsidian-800 cursor-text",
                className
            )}
        >
                ...
                <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
                    <div className="relative mb-1 w-fit rounded-[1rem] px-1 pt-1">
                        ...
                        <button
                            ...
                            className="absolute right-2 top-2 z-10 flex h-4 w-4 items-center justify-center rounded-full bg-obsidian-900 text-white transition-colors hover:bg-obsidian-800"
            ...
            <div className="relative w-full">
                {/* Background Underlay for Syntax Highlighting */}
                <div
                    className="absolute inset-0 p-3 text-sm font-sans whitespace-pre-wrap break-words pointer-events-none overflow-hidden z-0"
                    aria-hidden="true"
                >
                    {!actualValue && <span className="text-obsidian-400">Message...</span>}
                    {actualValue &&
                        actualValue.split(/(@ai)/gi).map((part, i) =>
                            part.toLowerCase() === "@ai" ? (
                                <span
                                    key={i}
                                    className="text-accent-violet font-bold bg-accent-violet/10 rounded px-0.5 shadow-[0_0_8px_rgba(99,102,241,0.3)]"
                                >
                                    {part}
                                </span>
                            ) : (
                ...
                <textarea
                    ...
                    onScroll={(e) => {
                        const underlay = e.target.previousElementSibling;
                        if (underlay) {
                            underlay.scrollTop = e.target.scrollTop;
                        }
                    }}
                    spellCheck={false}
                    className="custom-scrollbar w-full resize-none border-0 bg-transparent p-3 focus:ring-0 focus-visible:outline-none min-h-12 text-sm font-sans relative z-10 text-transparent caret-white placeholder-transparent"
                    {...props}
                />
            </div>
                    ...
                    <div className="flex items-center gap-2">
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <button
                                    ...
                                    className="flex h-8 w-8 items-center justify-center rounded-full text-obsidian-300 transition-colors hover:bg-obsidian-800 hover:text-white focus-visible:outline-none"
                                >
                        ...
                        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <PopoverTrigger asChild>
                                        <button
                                            type="button"
                                            className="flex h-8 items-center gap-2 rounded-full p-2 text-sm text-obsidian-300 transition-colors hover:bg-obsidian-800 hover:text-white focus-visible:outline-none focus-visible:ring-ring"
                                        >
                            ...
                            <PopoverContent side="top" align="start">
                                <div className="flex flex-col gap-1 bg-obsidian-950 p-1">
                                    {toolsList.map((tool) => (
                                        <button
                                            key={tool.id}
                                            onClick={() => {
                                            ...
                                            className="flex w-full items-center gap-2 rounded-md p-2 text-left text-sm text-obsidian-200 hover:bg-obsidian-850 hover:text-white"
                                        >
                                            <tool.icon className="h-4 w-4" />
                                            <span>{tool.name}</span>
                                            {tool.extra && (
                                                <span className="ml-auto text-xs text-obsidian-500">{tool.extra}</span>
                                            )}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {activeTool && (
                            <>
                                ...
                                <button
                                    ...
                                    className="flex h-8 items-center gap-2 rounded-full px-2 text-sm hover:bg-obsidian-800 cursor-pointer text-accent-violet transition-colors flex-row justify-center"
                                >
                        ...
                        <div className="ml-auto flex items-center gap-2">
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <button
                                        type="button"
                                        className="flex h-8 w-8 items-center justify-center rounded-full text-obsidian-400 transition-colors hover:bg-obsidian-800 hover:text-white focus-visible:outline-none"
                                    >
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <button
                                        ...
                                        disabled={!hasValue}
                                        className="flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-colors focus-visible:outline-none disabled:pointer-events-none bg-accent-violet text-white hover:bg-accent-violet/90 disabled:bg-obsidian-800 disabled:text-obsidian-500"
                                    >
                                        <SendIcon className="h-5 w-5 font-bold" />
